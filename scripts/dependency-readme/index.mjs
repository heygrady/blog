...
async function fetchAndSaveReadme(depName, version) {
  ...
  try {
    let { stdout: readme } = await execAsync(
      `npm view ${depName}@${versionString} readme`
    )

    if (!readme || readme.trim().length < 10) {
      console.log(
        `  🤔 README for ${depName}@${versionString} is blank. Trying GitHub...`
      )
      try {
        const { stdout: rawRepoUrl } = await execAsync(
          `npm view ${depName}@${versionString} repository.url`
        )
        ...
        if (match) {
          const repo = match[1]
          try {
            const { stdout: githubReadme } = await execAsync(
            ...
            const { stdout: githubReadme } = await execAsync(
              `curl -sL https://raw.githubusercontent.com/${repo}/master/README.md`
            )
            readme = githubReadme
        ...
        console.error(
          `  ❌ Failed to fetch README from GitHub for ${depName}@${versionString}: ${error.message}`
        )
      }
    }
    ...
    console.error(
      `  ❌ Failed to fetch README for ${depName}@${versionString}: ${error.message}`
    )
  }
}
...
async function main() {
  ...
  if (!arg) {
    console.error(
      'Usage: node scripts/dependency-readme/index.mjs <workspace-package-name | package-name | package-name@version>'
    )
    process.exit(1)
  }
  ...
  try {
    ...
    if (workspace) {
      ...
      console.log(
        `Found ${Object.keys(dependencies).length} dependencies for ${arg}`
      )
      ...
      for (const [depName, depVersion] of Object.entries(dependencies)) {
        ...
        if (depVersion.startsWith('workspace:')) {
          console.log(
            `  ⏩ Skipping versioned README for workspace dependency ${depName}`
          )
          continue
        }
